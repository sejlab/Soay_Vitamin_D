...
#tophits <- subset(gwas.results, Pc1df < 1e-4)
tophits <- subset(gwas.results, Pc1df < 1.28e-6)
...
write.table(toporthos, "results/3_Top_Orthologous_Regions_v3.txt", row.names = F, sep = "\t", quote = F)
write.table(topgenes, "results/3_Top_Genes_v3.txt", row.names = F, sep = "\t", quote = F)
...
vitdterms <- c("vitamin D", "cholecalcif", "ergoecalcif", "CYP2", "NADSYN1", "DHCR7", "SEC23A", #"GC",
               "AMDHD1")
...
vitdlines <- unique(vitdlines)

topGOvitd <- topGO[vitdlines,]
table(topGOvitd$external_gene_name, topGOvitd$Species)

write.table(topGO, "results/4_Top_GO_Terms_v3.txt", row.names = F, sep = "\t", quote = F)
write.table(topGOvitd, "results/4_Top_GO_Terms_vitd_v3.txt", row.names = F, sep = "\t", quote = F)
